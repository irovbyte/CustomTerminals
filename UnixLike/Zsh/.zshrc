# ============================================================
# 1. Instant Prompt (ускорение загрузки Powerlevel10k)
# ============================================================
if [[ -r "${XDG_CACHE_HOME:-$HOME/.cache}/p10k-instant-prompt-${(%):-%n}.zsh" ]]; then
  source "${XDG_CACHE_HOME:-$HOME/.cache}/p10k-instant-prompt-${(%):-%n}.zsh"
fi

# ============================================================
# 2. Oh My Zsh — базовая конфигурация
# ============================================================
export ZSH="$HOME/.oh-my-zsh"
ZSH_THEME="powerlevel10k/powerlevel10k"

# ============================================================
# 3. Плагины Oh My Zsh
# ============================================================
plugins=(
  git
  zsh-autosuggestions
  zsh-syntax-highlighting
)

source $ZSH/oh-my-zsh.sh

# ============================================================
# 4. Powerlevel10k — настройки темы
# ============================================================
[[ ! -f ~/.p10k.zsh ]] || source ~/.p10k.zsh

# ============================================================
# 5. irovbyte auto-restore system
# ============================================================
CONFIG_DIR="$HOME/.config/irovbyte"
CONFIG_FILE="$CONFIG_DIR/config.zsh"
REPO_URL="https://github.com/irovbyte/CustomTerminals.git"

# Проверка наличия конфига
if [ ! -f "$CONFIG_FILE" ]; then
    echo ""
    echo "⚠️  Конфиг irovbyte отсутствует."
    echo "Без него вы не будете получать новые обновления и алиасы."
    echo ""
    read -p "📥 Скачать и восстановить конфиг? (y/n): " answer

    if [[ "$answer" == "y" || "$answer" == "Y" ]]; then
        echo "🔄 Восстанавливаю конфиги irovbyte..."
        rm -rf "$CONFIG_DIR" 2>/dev/null
        git clone "$REPO_URL" "$CONFIG_DIR"
        echo "✅ Конфиг успешно восстановлен!"
    else
        echo "⛔ Конфиг не восстановлен. Новые обновления irovbyte применяться не будут."
        echo ""
    fi
fi

# Подключение конфига
if [ -f "$CONFIG_FILE" ]; then
    source "$CONFIG_FILE"
fi


# ============================================================
# UNI-SENTINEL (Auto-Installer & Alias)
# ============================================================
uni-sentinel() {
    if [ ! -d "$HOME/.uni-sentinel" ]; then
        echo "⚠️  Uni-Sentinel не найден."
        echo "📥 Запускаю установку..."
        # Замени irovbyte на свой ник, если он отличается в ссылке
        bash <(curl -s https://raw.githubusercontent.com/irovbyte/Uni-Sentinel/main/install.sh)
    else
        python3 "$HOME/.uni-sentinel/main.py" "$@"
    fi
}